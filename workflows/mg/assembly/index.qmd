...
lightbox: true
listing: 
  id: snaklemake-workflow
  contents: include/snaklemake-workflow.yaml
  type: table
  sort-ui: false
  filter-ui: false
  fields: [name, tool, description, input, reference]
  field-links: 
    - tool
  field-display-names: 
    name: Task
...
In this section of the workflow we begin with raw, paired-end Illumina data. We will use a Snakemake [@koster2012snakemake] workflow within the anvi'o [@eren2021community] environment for most of the steps, though we will at times call on additional tools for specific steps. The overall structure of our workflow was modeled after the one described by Delmont & Eren on [Recovering Microbial Genomes from TARA Oceans Metagenomes](http://merenlab.org/data/tara-oceans-mags/) [@delmont2018nitrogen].

The main steps of this workflow (in order) are:

::: {#snaklemake-workflow .column-body}
:::
...
Thanks to the anvi'o implementation of Snakemake we can run many commands sequentially and/or simultaneously. There is plenty of good documentation on the anvi'o website about [setting up Snakemake workflows](http://merenlab.org/2018/07/09/anvio-snakemake-workflows/) so we will refrain from any lengthy explanations.

You can see all the settings we used and [download our configs file](include/default_mg.json) or grab it from the folded code below. Some commands we chose not to run but they remain in the workflow for posterity.
...
Since we are doing a co-assembly, we need a separate file called [`samples.txt`](include/samples.txt). This file tells anvi'o where to find the **raw** fastq files for each sample and what group the sample belongs to. The file is a four-column, tab-delimited file.
...
#| fig-subcap:
#|   - "DAG of the metagenomic workflow for a small subset of samples."
...
There are many tools used in the workflow that need to be cited. See the [Workflow References](/workflows/pub/references/) page for details. 
...
::: {.callout-note title="irSorter2 column values"}
